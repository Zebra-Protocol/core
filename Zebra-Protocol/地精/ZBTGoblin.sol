...
contract ZBTGoblin is Ownable, ReentrancyGuard, Goblin {
    /// @notice Libraries
        ...
        nonReentrant
    {
        ...
        if (borrow > 0 && borrowToken != address(0)) {
            ...
            IERC20(borrowToken).safeApprove(address(strategy), 0);
            IERC20(borrowToken).safeApprove(address(strategy), uint256(-1));
        }
    ...
    function health(uint256 id, address borrowToken) external override view returns (uint256) {
        ...
        nonReentrant
    {
        ...
        if (isBorrowHt){
            ...
            safeTransferETH(msg.sender, tokenLiquidate);
        } else {
            tokenLiquidate = myBalance(borrowToken);
            IERC20(borrowToken).safeTransfer(msg.sender, tokenLiquidate);
        }
